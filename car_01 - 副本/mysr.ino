...
float a[6]={
};    //前置超声波传感器数据缓存：存取6个测距数据：分别为a[0]、a[1]、a[2]、a[3]、a[4]、a[5]
float b[6]={
};    //左侧超声波传感器数据缓存：存取6个测距数据
float c[6]={
};    //右侧超声波传感器数据缓存：存取6个测距数据
...
//读取前置超声波测距数据
void read_csb(int TrigPin,int EchoPin, int FangXiang)
{
  unsigned int s,t,f;   
  ...
  for (s=0;s<6;s++)         //连续6次，读取前置超声波测距数值
  {
    ...
    switch (FangXiang)
    {
      //1 前方
    case 1:   
      a[s] = pulseIn(EchoPin, HIGH) / 58.00;   //计算出测距数值，存放于前置超声波数组中
      break;

      //2 左侧            
    case 2:   
      b[s] = pulseIn(EchoPin, HIGH) / 58.00;
      break;
      //3 右侧
    case 3:  
      c[s] = pulseIn(EchoPin, HIGH) / 58.00;
      break;
    }
